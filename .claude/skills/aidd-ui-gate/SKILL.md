...
**Nguyên tắc bất di:**
- **Visual (nhóm A) = "đủ giống ~95%"**, KHÔNG bắt pixel-perfect. Lệch nhỏ px spacing / sắc độ màu → bỏ qua. Chỉ FAIL khi lệch RÕ (sai layout/cấu trúc, sai màu hẳn, thiếu/sai vị trí element, sai font, chữ tràn/vỡ). Vẫn lấy số từ MoMorph MCP làm mốc — không bịa.
...
**Precondition:** screen phải **render được ở route local với mock data** (FE đã build UI+behavior mock theo `fe-developer.md`) và **dev server sống** (`npm run dev`, http://localhost:3000). Chưa thỏa → xem Error Recovery.
...
### 3. Visual-diff (nhóm A)
Với mỗi viewport: `browser_resize(w, h)` → `browser_navigate` route → `browser_take_screenshot`.

**Cách chấm (không có pixel-diff tự động — dùng model-visual + đối chiếu số):**
**Chụp FULL PAGE (`fullPage:true`) — soi TỪ HEADER TỚI FOOTER, mọi section, không chỉ vùng nhìn thấy.** (Đã từng sót hẳn footer.)

1. Đặt screenshot cạnh Figma reference (Step 2), soi từng vùng theo checklist:
...
2. Vùng nghi ngờ → đọc giá trị thật (`browser_evaluate` + `getComputedStyle`; icon SVG đọc `fill`/`stroke`) đối chiếu node spec.

**Ngưỡng PASS/FAIL (visual "đủ giống ~95%" — KHÔNG pixel-perfect):**

| Viewport | Ngưỡng |
|---|---|
| **1440 (~95% giống)** | **FAIL khi lệch RÕ:** sai **layout composition** (full-width vs 2-cột, nhóm section sai) · sai **tỉ lệ/kích thước** box · sai màu hẳn (khác hue) · **sai màu icon** · logo/wordmark/artwork **dựng bằng text/CSS thay vì asset ảnh** · sai font-family/weight · thiếu/sai vị trí element · **element/text đè/cắt** · **mock data quá thưa so với Figma** · thiếu section (vd footer). **BỎ QUA:** lệch spacing/size vài px, chênh sắc độ nhỏ. |
| 768 / 375 (adapt) | **FAIL** nếu: overflow ngang (scroll ngang) · chồng/tràn chữ · element bị cắt · layout vỡ. KHÔNG chấm lệch px vs design. |

Report ghi từng FAIL kèm bằng chứng (vùng nào, sai cái gì). Lệch nhỏ đã bỏ qua thì không cần liệt kê.
...
## A. Visual ~95% (1440 giống / 768 / 375 adapt) — chấm full-page
- Layout composition (full-width vs 2-cột): PASS|FAIL
- Kích thước/tỉ lệ · màu · màu icon · asset ảnh (không fake text) · không đè/cắt · density mock · đủ section (footer): {liệt kê FAIL}
- 1440: PASS|FAIL · 768: PASS|FAIL · 375: PASS|FAIL
...
- **PASS** khi: A đạt ~95% (không lệch rõ) **VÀ** B đúng 100%. → screen được phép sang integration → test → review. Báo user.
- Lưu ý: B sai 1 mục → **FAIL toàn gate** kể cả A đẹp. Visual lệch nhỏ → vẫn PASS.
...
- 1440 = chuẩn visual ~95% (không pixel-perfect). 1280 KHÔNG chấm. 768/375 chỉ chấm responsive-ok.
...
- Visual nhân nhượng lệch nhỏ; behavior KHÔNG nhân nhượng — đây là ưu tiên cao nhất của gate.
...
- **Port:** dev thường ở `localhost:3000` nhưng có thể bị process khác chiếm/`::1` → dùng `127.0.0.1:{port}` hoặc port dev thực tế; ghi port đã dùng vào report.
