 ...
 */
// '/countdown' is public so anonymous visitors can see the pre-launch countdown
// they are redirected to (the launch gate sends everyone there before launch).
export const PUBLIC_PATHS = ['/', '/login', '/auth', '/dev-login', '/countdown'] as const

export function isPublic(pathname: string): boolean {
  return PUBLIC_PATHS.some((p) => pathname === p || pathname.startsWith(`${p}/`))
}

/** Chống open-redirect: chỉ path nội bộ bắt đầu bằng "/" (không "//"), mặc định / (Homepage). */
